...
| 2 | **Q-61** | 0.6.0 readability (code shape) | code | Named extracts in [`code-shape.md`](./code-shape.md). Stretch (does not block 0.6.0): typed rejects. **Today: 0.6.0 named extracts + IBD confirm-event drain + ChainHub `HeldBodies` + Query `ShWriteBehind` + mempool admit/evict helpers + one-shot load = stamp+load_from_plan in Completed.** Stretch remains. |
...
| **Q-61** | Keep. Named 0.6.0 extracts landed (Completed). IBD confirm-event drain, ChainHub `HeldBodies`, Query `ShWriteBehind`, mempool admit/evict helpers, and one-shot load = stamp+load_from_plan landed. Stretch still open (typed rejects). Interpreter opcode match and io_uring stay dense. |
