...
| 2 | **Q-61** | 0.6.0 readability (code shape) | code | Named extracts in [`code-shape.md`](./code-shape.md). Stretch (does not block 0.6.0): IBD confirm-drain once, ChainHub has-a, Query SH write-behind type, confirm pin split, typed rejects. **Today: 0.6.0 named extracts in Completed.** Stretch remains. |
...
| **Q-61** | Keep. Named 0.6.0 extracts landed (Completed). Stretch still open. Interpreter opcode match and io_uring stay dense. |
...
| ID | Item | Resolution |
|----|------|------------|
| **—** | Q-61 0.6.0 named extracts | Display-hash owner (`display_hash_hex` / `parse_display_hash32`); CLI `CliAccum` / `apply_kv`; RPC `METHOD_LIST` catalog so `help` / `getrpcinfo` list every dispatched method; Electrum/Esplora `sh_at_view`; `PeerFollowState` + `PendingSendCmpct` + shared mempool GetData; `CatchUp` + named `run_p2p` phases + shared hub-tip bridge. Stretch stays Open. |
