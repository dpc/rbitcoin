...
| 2 | **Q-61** | 0.6.0 readability (code shape) | code | Named extracts in [`code-shape.md`](./code-shape.md). Stretch (does not block 0.6.0): Query SH write-behind type, confirm pin split, typed rejects. **Today: 0.6.0 named extracts + IBD confirm-event drain + ChainHub `HeldBodies` in Completed.** Stretch remains. |
...
| **Q-61** | Keep. Named 0.6.0 extracts landed (Completed). IBD confirm-event drain and ChainHub `HeldBodies` landed. Stretch still open. Interpreter opcode match and io_uring stay dense. |
