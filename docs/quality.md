...
| 2 | **Q-61** | 0.6.0 readability (code shape) | code | Named extracts in [`code-shape.md`](./code-shape.md). Stretch (does not block 0.6.0): confirm pin split, typed rejects. **Today: 0.6.0 named extracts + IBD confirm-event drain + ChainHub `HeldBodies` + Query `ShWriteBehind` + mempool admit/evict helpers in Completed.** Stretch remains. |
...
| 8 | **Q-31** | Hermetic tip fixtures | ops | Frozen signet/mainnet tip packs for offline consensus/Electrum regression (no live API). **Fuzz corpora** now merge tiny `signet_block_*.bin` / `mainnet_block_290329.bin` (remined onto regtest). Electrum hermetic packs still Open. |
| 9 | **R-10** | Residual god-files | code | Peel **only** when a higher row needs a seam, **or** when **Q-61** names the extract. Unnamed line-count peels still wait. Do not split `interpreter.rs` opcode `match` or io_uring machines. Production leftover (2026-09-03 `wc -l`): `electrum/server` **5.8k**, `query/lib` **5.1k**, `chain` **3.9k**, `rpc/methods` **3.6k**, `scripthash` **3.5k**, `store` **3.4k**, `peer` **2.9k**, `ibd/perf_log` **2.9k**, `interpreter` **2.7k**. `sorted_run` peeled to **1.3k**. **Q-54** may need a seam if a cap rule cannot match a god-file. |
...
later (0.6.0 readability); 2026-09-03 remaining holes already had Open rows.
...
| **Q-61** | Keep. Named 0.6.0 extracts landed (Completed). IBD confirm-event drain, ChainHub `HeldBodies`, Query `ShWriteBehind`, and mempool admit/evict helpers landed. Stretch still open. Interpreter opcode match and io_uring stay dense. |
...
| **Q-60** | Keep rank 6. Prefill monotonic + `held_seq` + WS `spawn_blocking` + outbound asmap/prefix diversity landed. AddrMan caps and `announced_wtx` roll still open. |
| **Q-48** | Keep, rank 7. Waits on rust-bitcoin (**RB-007**). |
| **Q-31** | Keep. Feeds nightly fuzz corpora; not blocking operators. |
| **R-10** | Keep. Unnamed peels wait; **Q-61** names the 0.6.0 extracts. Largest production file is now `electrum/server` **5.8k** (tweaks subscribe). `sorted_run` peeled to **1.3k**. |
...
| ID | Item | Resolution |
|----|------|------------|
| **—** | Q-61 0.6.0 named extracts | Display-hash owner (`display_hash_hex` / `parse_display_hash32`); CLI `CliAccum` / `apply_kv`; RPC `METHOD_LIST` catalog so `help` / `getrpcinfo` list every dispatched method; Electrum/Esplora `sh_at_view`; `PeerFollowState` + `PendingSendCmpct` + shared mempool GetData; `CatchUp` + named `run_p2p` phases + shared hub-tip bridge. IBD confirm events drain through `apply_confirm_events` (call sites kept); Headers apply is named stages; dead `ConfirmEvent::Reject.wire` dropped. `ChainHub` holds `HeldBodies` under one `RwLock` (cap 320, lowest first-seen seq evicted); `Invalidated` / `HeaderTips` / `MiningKnobs` are named types. Query SH write-behind is `ShWriteBehind` (locks unchanged); `IndexMode` names archive-spend / SH-enqueue products; `TxTable::probe_body_match_fk` is the body-txid head probe. Mempool `scan_conflicts_and_parents` / `evict_worst_chunks` / hub `admit_staged` (write-lock re-check kept). Stretch stays Open. |
...
| God-file peels named by **Q-61**, or when a higher Open row needs a seam (**R-10**) | Split `interpreter.rs` opcode `match` / io_uring machines / MPHF to beat a line count |
...
| Audience | Read |
|----------|------|
| Next quality slice | **Open**, rank 1 (**Q-41** Core functional `run` set). 0.6.0 named extracts landed; **Q-61** stretch remains ([`code-shape.md`](./code-shape.md)). Folded store/mempool/RPC/P2P leftovers: **Q-57–Q-60**. Next unused Q-id is **Q-62** |
