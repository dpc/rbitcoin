...
| 2 | **Q-61** | 0.6.0 readability (code shape) | code | Named extracts in [`code-shape.md`](./code-shape.md) landed as separate PRs: one display-hash owner, CLI flag table, RPC method catalog (`help` lists every dispatched method), Electrum/Esplora session + `sh_at_view`, `PeerFollowState` + per-message handlers, `run_p2p` phases. Stretch (does not block 0.6.0): IBD confirm-drain once, ChainHub has-a, Query SH write-behind type, confirm pin split, typed rejects. **Today: owner doc; display-hash owner landed.** Close a named extract by moving that sentence into Completed in the same PR. |
...
| **Q-61** | **Opened.** Owner [`code-shape.md`](./code-shape.md). Rank 2 for the 0.6.0 window. Named extracts only; interpreter opcode match and io_uring stay dense. |
...
| Audience | Read |
|----------|------|
| Next quality slice | **Open**, rank 1 (**Q-41** Core functional `run` set). 0.6.0 readability: **Q-61** ([`code-shape.md`](./code-shape.md)). Folded store/mempool/RPC/P2P leftovers: **Q-57–Q-60**. Next unused Q-id is **Q-62** |
